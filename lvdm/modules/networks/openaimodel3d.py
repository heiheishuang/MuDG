...
class UNetModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 default_fs=4,
                 fs_condition=False,
                 class_label_condition=False,
                 domain_cross_attention=False,
                 num_tasks=1,
                 temporal_frozen=False
                ):
        ...
        self.class_label_condition = class_label_condition
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        SpatialTransformer(ch, num_heads, dim_head,
                            ...
                            domain_cross_attention=self.domain_cross_attention, num_tasks=self.num_tasks
                        )
                    )
    ...
    def forward(self, x, timesteps, c_label=None, context=None, features_adapter=None, fs=None, **kwargs):
        b,_,t,_,_ = x.shape
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False).type(x.dtype)
        emb = self.time_embed(t_emb)

        if self.class_label_condition:
            assert c_label is not None, 'class_label is required for class_label_condition'
            class_emb = timestep_embedding(c_label, self.model_channels, repeat_only=False).type(x.dtype)
            class_emb = self.class_embed(class_emb)
            emb = emb + class_emb
