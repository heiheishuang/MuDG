...
class JointCrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.,
                 relative_position=False, temporal_length=None, video_length=None, text_context_len=77,
                 num_tasks=1):
        super().__init__()
        inner_dim = dim_head * heads
        context_dim = default(context_dim, query_dim)
    ...
    def init_weight(self):
        # init weight for to_q, to_k, to_v
        q_weight = nn.Parameter(torch.zeros_like(self.to_q.weight))
        self.to_q.weight = nn.Parameter(q_weight)
        k_weight = nn.Parameter(torch.zeros_like(self.to_k.weight))
        self.to_k.weight = nn.Parameter(k_weight)
        v_weight = nn.Parameter(torch.zeros_like(self.to_v.weight))
        self.to_v.weight = nn.Parameter(v_weight)

        # init weight and bias for to_out
        out_weight = nn.Parameter(torch.zeros_like(self.to_out[0].weight))
        self.to_out[0].weight = nn.Parameter(out_weight)


    def forward(self, x, context=None, mask=None):
        spatial_self_attn = (context is None)
        assert spatial_self_attn is True
    ...
    def split_tasks(self, k, v):

        k_tuple = torch.chunk(k, dim=0, chunks=self.num_tasks) # [b, n, d] -> [b / M, n, d]
        v_tuple = torch.chunk(v, dim=0, chunks=self.num_tasks) # [b, n, d] -> [b / M, n, d]
        k = torch.cat(k_tuple, dim=1) # [b / M, M * n, d]
        v = torch.cat(v_tuple, dim=1) # [b / M, M * n, d]
        k = torch.cat([k] * self.num_tasks, dim=0) # [b, M * n, d]
        v = torch.cat([v] * self.num_tasks, dim=0) # [b, M * n, d]
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True,
                disable_self_attn=False, attention_cls=None, video_length=None, image_cross_attention=False,
                 ...
                 domain_cross_attention=False, num_tasks=1):
        super().__init__()
        ...
        self.domain_cross_attention = domain_cross_attention
        if self.domain_cross_attention:
            assert num_tasks > 1
            self.attn_joint = JointCrossAttention(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout,
                                                  num_tasks=num_tasks)
            self.norm_joint = nn.LayerNorm(dim)

            # init weight for attn_joint and norm_joint
            self.attn_joint.init_weight()
            norm_joint_weight = nn.Parameter(torch.zeros_like(self.norm_joint.weight))
            self.norm_joint.weight = nn.Parameter(norm_joint_weight)
    ...
    def _forward(self, x, context=None, mask=None):
        ...
        if self.domain_cross_attention:
            x = self.attn_joint(self.norm_joint(x), context=None, mask=mask) + x
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
        ...
        self.transformer_blocks = nn.ModuleList([
            BasicTransformerBlock(
                ...
                domain_cross_attention=domain_cross_attention,
                num_tasks=num_tasks,
...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 ...
                 relative_position=False, temporal_length=None, temporal_frozen=False):
        super().__init__()
