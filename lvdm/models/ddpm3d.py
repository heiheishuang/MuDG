...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, **kwargs):
        ...
        class_label = kwargs.get('class_label', None)[:, 0]
        x_recon = self.model(x_noisy, t, class_label, **cond, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        # sparse_x = kwargs.get('sparse_x', None) # B C T H W
        # x_noisy = torch.cat([x_noisy, sparse_x], dim=1) if sparse_x is not None else x_noisy
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        x, sparse_x, c, fs, class_label = self.get_batch_input(batch, random_uncond=random_uncond,
                                                               return_fs=True, return_class_label=True)
        kwargs.update({"fs": fs.long()})
        kwargs.update({"sparse_x": sparse_x})
        kwargs.update({"class_label": class_label})
        loss, loss_dict = self(x, c, **kwargs)
        return loss, loss_dict
    
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False,
                        return_fs=False, return_cond_frame=False, return_original_input=False, return_sparse_input=False,
                        return_class_label=False,
                        **kwargs):
        # batch = self.get_processed_batch(batch)
        ## x: b c t h w
        x = super().get_input(batch, self.first_stage_key)
        sparse_x = super().get_input(batch, "sparse_frames")
        class_label = super().get_input(batch, "class_label")
        sparse_depth = super().get_input(batch, "sparse_depth")

        ## encode video frames x to z via a 2D encoder        
        z = self.encode_first_stage(x)
        sparse_z = self.encode_first_stage(sparse_x)
        sparse_depth_z = self.encode_first_stage(sparse_depth)
        ...
        if self.rand_cond_frame:
            assert self.rand_cond_frame is False, "random condition frame is not supported"
            cond_frame_index = random.randint(0, self.model.diffusion_model.temporal_length-1)

        img = sparse_x[:,:,cond_frame_index,...]
        img = input_mask * img
        ## img: b c h w
    ...
    def get_processed_batch(self, batch):
        ## TBD: process batch to get processed batch
        # concat multi task data
        bs = batch['color_dense_frames'].shape[0]
        tasks = batch['tasks']

        multi_task = self.model.diffusion_model.num_tasks
        assert multi_task == len(batch['tasks']) + 1

        dense_frames = [batch['color_dense_frames']] + [task['dense_frames'][0:bs] for task in tasks]
        sparse_frames = [batch['color_sparse_frames']] + [task['sparse_frames'][0:bs] for task in tasks]
        fps = [batch['color_fps']] + [task['fps'][0:bs] for task in tasks]
        class_label = [batch['color_class_label']] + [task['class_label'][0:bs] for task in tasks]

        dense_frames = torch.cat(dense_frames, dim=0)
        sparse_frames = torch.cat(sparse_frames, dim=0)
        fps = torch.cat(fps, dim=0)
        class_label = torch.cat(class_label, dim=0)


        caption = [*batch['color_caption']]
        for task in tasks:
            caption = caption + [*(task['caption'][0:bs])]
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
        ...
        z, sparse_z, c, xrec, xc, fs, cond_x, sparse, class_label = self.get_batch_input(
            batch,
            random_uncond=False,
            return_first_stage_outputs=True,
            return_original_cond=True,
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_label=None, c_concat: list = None, c_crossattn: list = None,
        ...
        if self.conditioning_key is None:
            ...
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, c_label=c_label, context=cc, **kwargs)
