...
from tools.filter_dynamic import stat_dynamic_objects
from tools.waymo_dataset import *
...
def process_single_sequence(
    ...
    from tools.waymo_filereader import WaymoDataFileReader
                ...
                if should_process_gt and (lidar_dir or pcl_dir):
                    ...
                    if laser.name == dataset_pb2.LaserName.TOP:
                        range_image_top_pose_str_tensor = tf.io.decode_compressed(
                            laser_return.range_image_pose_compressed, 'ZLIB')
                        ...
                        frame_pose_local = tf.expand_dims(
                            tf.convert_to_tensor(np.reshape(np.array(frame.pose.transform), [4, 4])), axis=0)
                    else:
                    ...
                    if should_process_gt and lidar_dir:
                        ...
                        if pixel_pose_local is not None:
                            ...
                            np.savez_compressed(lidar_cur_fpath, rays_o=rays_o.numpy().astype(np.float32),
                                                rays_d=rays_d.numpy().astype(np.float32),
                                                ranges=_range_image_range.astype(np.float32),
                                                dpose=dpose.astype(np.float32))
                        else:
                    ...
                    if should_process_gt and pcl_dir:
                        ...
                        points_intensity_tensor = tf.gather_nd(range_image_intensity,
                                                               tf.compat.v1.where(range_image_mask))
                        points_elongation_tensor = tf.gather_nd(range_image_elongation,
                                                                tf.compat.v1.where(range_image_mask))
...
if __name__ == "__main__":
    ...
    create_dataset(args.root, args.seq_list, args.out_root, j=args.j, should_offset_pos=not args.no_offset_pose,
                   should_process_gt=not args.no_process_gt, ignore_existing=args.ignore_existing)