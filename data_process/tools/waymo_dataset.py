...
def clip_node_data(odict: dict, start, stop):
    ...
    if (start is None and stop is None) or (
            'global_frame_inds' not in odict['data'] and 'global_timestamps' not in odict['data']):
        t = None
    else:
        if 'global_frame_inds' in odict['data']:
            assert (start is None or isinstance(start, int)) and (
                    stop is None or isinstance(stop, int)), f"Please use frame ind clipping."
            t = odict['data']['global_frame_inds']
        else:
            assert (start is None or isinstance(start, float)) and (
                    stop is None or isinstance(stop, float)), f"Please use timestamp clipping."
...
def clip_node_segments(odict: dict, start, stop):
    ...
    for seg in old_segs:
        if (start is None and stop is None) or (
                'global_frame_inds' not in seg['data'] and 'global_timestamps' not in seg['data']):
            new_segs.append(seg)
            continue
        else:
...
class WaymoDataset(SceneDataset):
    ...
    def get_image_mono_depth(self, scene_id: str, camera_id: str, frame_index: int) -> np.ndarray:
        fpath = os.path.join(self.root, scene_id, self.image_mono_depth_dirname, camera_id,
                             f'{idx_to_frame_str(frame_index)}.npz')
        assert os.path.exists(fpath), f"Not exist: {fpath}"
    ...
    def get_image_mono_normals(self, scene_id: str, camera_id: str, frame_index: int) -> np.ndarray:
        fpath = os.path.join(self.root, scene_id, self.image_mono_normals_dirname, camera_id,
                             f'{idx_to_frame_str(frame_index)}.jpg')
        assert os.path.exists(fpath), f"Not exist: {fpath}"
    ...
    def get_raw_mask(self, scene_id: str, camera_id: str, frame_index: int, *, compress=True):
        fpath = os.path.join(self.root, scene_id, self.mask_dirname, camera_id,
                             idx_to_mask_filename(frame_index, compress=compress))
        assert os.path.exists(fpath), f"Not exist: {fpath}"
        # [H, W]
        if compress:
    ...
    def get_image_semantic_mask_all(self, scene_id: str, camera_id: str, frame_index: int, *,
                                    compress=True) -> np.ndarray:
        raw = self.get_raw_mask(scene_id, camera_id, frame_index, compress=compress)
    ...
    def get_semantic_mask_of_class(self, scene_id: str, camera_id: str, frame_index: int, dataset_class_str: str, *,
                                   compress=True) -> np.ndarray:
        raw = self.get_raw_mask(scene_id, camera_id, frame_index, compress=compress)
        ret = np.zeros_like(raw).astype(np.bool8)
        for cls in self.dataset_classes_in_sematic[dataset_class_str]:
    ...
    #                     [seg['data']['transform'][seg_local_fi][:3, :].reshape(-1), seg['data']['scale'][seg_local_fi]])
    ...
    #         if cfg.get('dynamic_only', False) and (
    #                 oid not in scenario['metas']['dynamic_stats'][o_class_name]['is_dynamic']):
